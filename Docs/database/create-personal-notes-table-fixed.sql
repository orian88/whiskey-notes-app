-- 개인 노트 테이블 생성 (수정된 버전)
-- 'date' 컬럼명을 'note_date'로 변경하여 PostgreSQL 예약어 충돌 방지
...
-- 인덱스 생성
CREATE INDEX IF NOT EXISTS idx_personal_notes_date ON personal_notes(note_date);
CREATE INDEX IF NOT EXISTS idx_personal_notes_category ON personal_notes(category);
