-- 위스키 가격 추적 테이블 추가
-- 이 SQL 스크립트는 whiskeys 테이블에 가격 추적 기능을 추가합니다
...
-- 인덱스 생성 (이미 존재하는 경우 DROP 후 재생성)
DROP INDEX IF EXISTS idx_whiskey_prices_whiskey_id;
DROP INDEX IF EXISTS idx_whiskey_prices_date;
DROP INDEX IF EXISTS idx_whiskey_prices_whiskey_date;

CREATE INDEX idx_whiskey_prices_whiskey_id ON whiskey_prices(whiskey_id);
CREATE INDEX idx_whiskey_prices_date ON whiskey_prices(price_date);
CREATE INDEX idx_whiskey_prices_whiskey_date ON whiskey_prices(whiskey_id, price_date DESC);

-- 2. whiskeys 테이블에 가격 관련 컬럼 추가
ALTER TABLE whiskeys 
ADD COLUMN IF NOT EXISTS current_price DECIMAL(10,2),
ADD COLUMN IF NOT EXISTS current_price_usd DECIMAL(10,2), -- USD 가격
ADD COLUMN IF NOT EXISTS exchange_rate DECIMAL(10,4), -- 환율 (KRW/USD)
