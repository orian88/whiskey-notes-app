...
interface MobileWhiskeyCartTabProps {
  searchTerm: string;
  filterBrand: string;
  filterType: string;
  filterRegion: string;
...
const MobileWhiskeyCartTab: React.FC<MobileWhiskeyCartTabProps> = ({
  searchTerm,
  filterBrand,
  filterType,
  filterRegion,
  ...
  const handleScroll = useCallback(() => {
    if (containerRef.current) {
      ...
      // localStorage에 저장
      sessionStorage.setItem('whiskeyCartScroll', String(scrollPositionRef.current));
    }
  }, []);
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (containerRef.current) {
        const savedScroll = sessionStorage.getItem('whiskeyCartScroll');
  ...
  const loadData = useCallback(async (reset: boolean = false) => {
    try {
      ...
      let query = supabase
        ...
        .eq('is_favorite', true) // 즐겨찾기된 위스키만 조회
        ...
        <div style={{ backgroundColor: 'white', padding: '6px', gap: '6px' }}>
          {whiskeys.map((whiskey, index) => (
            <div 
              ...
              style={{ borderBottom: index < whiskeys.length - 1 ? '1px solid #E5E7EB' : 'none' }}
            >
              <div
...
export default MobileWhiskeyCartTab;
